#! -*- coding: utf-8 -*-
# 主要模型
# This code come from "https://github.com/bojone/bert4keras"

import numpy as np
from passbert.layers import *
from passbert.snippets import insert_arguments
from passbert.snippets import delete_arguments
from passbert.snippets import is_string
from passbert.snippets import orthogonally_resize
...
class Transformer(object):
    ...
    def build(
        ...
        **kwargs
        ...
        debug(f"inpus at the begining of the build process: {inputs}")
    ...
    def call(self, inputs):
        """定义模型的执行流程
        """
        debug(f"before empeddings, the inputs are: {inputs}")
        # Embedding
        outputs = self.apply_embeddings(inputs)
        debug(f"after embeddings, the outpus for main layers are: {outputs}")
        # Main
        debug("We have %d hidden layers" % self.num_hidden_layers)
...
class LM_Mask(object):
    ...
    def compute_attention_bias(self, inputs=None):
        ...
        debug("We compute attention bias for LM Mask here")
...
class BERT(Transformer):
    ...
    def apply_embeddings(self, inputs):
        ...
        if self.embedding_size != self.hidden_size:
            debug(
                f"self.embedding_size = {self.embedding_size}, but self.hidden_size = {self.hidden_size}")
            x = self.apply(
                inputs=x,
                layer=Dense,
    ...
    def apply_main_layers(self, inputs, index):
        ...
        debug("apply_main_layers for bert")
        x = inputs
        z = self.layer_norm_conds[0]
...
class MixedBERT(BERT):
    ...
    def __init__(
        ...
        **kwargs  # 其余参数
        ...
        super(MixedBERT, self).__init__(**kwargs)
        if self.with_nsp and not self.with_pool:
            self.with_pool = True

        # GPT 的 __init__ 添加了 final_activation='softmax'
    ...
    def build(
        ...
        **kwargs
        ...
        这里构建两个模型，分别是 BERT 和 GPT，
        将 input 处理成 input0 for BERT 和 input1 for GPT，
        然后分别走 BERT 和 GPT 的构建流程。
        """
        debug("build model")
        if self.built:
            return None
        ...
        for i in range(self.num_hidden_layers):
            outputs1 = self.apply_main_layers(outputs, i)
        outputs1 = self.apply_final_layers(outputs1)
        ...
        outputs2 = embedding(inputs)
        for i in range(self.num_hidden_layers):
            outputs2 = self.apply_main_layers(outputs2, i)
        outputs2 = self.apply_final_layers(outputs2)
...
def build_mixed_transformer_model(
        config_path=None,
        return_keras_model=True,
        **kwargs):
    """
